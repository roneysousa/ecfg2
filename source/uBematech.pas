...
//Declaração das Funções da BemaFI32.dll em Delphi   
...
function Bematech_FI_LeituraMemoriaFiscalData( DataInicial: String; DataFinal: String ):  
...
Uses Forms,Windows, Messages, SysUtils,Dialogs;
...
begin
    if iRetorno = 0 then
       aMensagem := 'Erro de Comunicação !';

    If iRetorno = -1 Then
       aMensagem := 'Erro de Execução na Função. Verifique!';

    if iRetorno = -2  then
       aMensagem := 'Parâmetro Inválido !';

    if iRetorno = -3  then
       aMensagem := 'Alíquota não programada !';

    If iRetorno = -4 Then
       aMensagem := 'Arquivo BemaFI32.INI não encontrado. Verifique!';

    If iRetorno = -5 Then
       aMensagem := 'Erro ao Abrir a Porta de Comunicação';

    If iRetorno = -6 Then
       aMensagem := 'Impressora Desligada ou Desconectada';

    If iRetorno = -7 Then
       aMensagem := 'Banco Não Cadastrado no Arquivo BemaFI32.ini';

    If iRetorno = -8 Then
       aMensagem := 'Erro ao Criar ou Gravar no Arquivo Retorno.txt ou Status.txt';

    if iRetorno = -18 then
       aMensagem := 'Não foi possível abrir arquivo INTPOS.001 !';

    if iRetorno = -19 then
       aMensagem := 'Parâmetro diferentes !';

    if iRetorno = -20 then
       aMensagem := 'Transação cancelada pelo Operador !';

    if iRetorno = -21 then
       aMensagem := 'A Transação não foi aprovada !';

    if iRetorno = -22 then
       aMensagem := 'Não foi possível terminal a Impressão !';

    if iRetorno = -23 then
       aMensagem := 'Não foi possível terminal a Operação !';

    if iRetorno = -24 then
       aMensagem := 'Forma de pagamento não programada.';

    if iRetorno = -25 then
       aMensagem := 'Totalizador não fiscal não programado.';

    if iRetorno = -26 then
       aMensagem := 'Transação já Efetuada !';

    if iRetorno = -28 then
       aMensagem := 'Não há Informações para serem Impressas !';
end;
