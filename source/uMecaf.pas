...
const dll = 'ECFAFRAC.DLL';
{
  ...
  function AFRAC_AbrirDia(Data : pchar): Integer; StdCall; External dll;
    ...
    function NumeroCupom(var NC: string): Integer;
    function VersaoFirmware(): String;
    function VersaoDll(): String;
    function NumeroSerie(): String;
    function DataMovimento(): String;
...
var sNc: string;
    res : boolean;
begin
  res := InfoECF (sNc, '023', 'o Número do Cupom');
  Nc:= sNc;
  result := StrtoInt(Nc);
end;
...
function Informar(Operador, MsgCupom,
...
begin
  ...
  if (nome_func <> '') then
//     aMensagem(Format('%-40.40s %-6.6s %-50.50s' , [nome_func, codErr, msg]));
end;
...
Var
    ...
    res : boolean;
begin
  res := InfoECF (sNc, '003', 'a Versão do firmware');
  result := sNc;
End;
...
Var
    ...
    res : boolean;
begin
  res := InfoECF (sNc, '024', ' a Release da DLL');
  result := sNc;
End;
...
Var
    ...
    res : boolean;
begin
  res := InfoECF (sNc, '002', ' Número de série.');
  result := sNc;
End;
...
Var
    ...
    res : boolean;
begin
  res := InfoECF (sNc, '019', ' Data do movimento.');
  result := sNc;
End;
